...
const BorderGlow: React.FC<BorderGlowProps> = ({
    ...
    const updateGlow = useCallback(() => {
        ...
        if (!card || targetRef.current.x === null || targetRef.current.y === null) {
            rafRef.current = null;
            return;
        }
        ...
        const edge = getEdgeProximity(card, currentRef.current.x, currentRef.current.y);
        const angle = getCursorAngle(card, currentRef.current.x, currentRef.current.y);
    ...
    const handlePointerLeave = useCallback(() => {
        targetRef.current = { x: null, y: null };
        currentRef.current = { x: null, y: null };
        if (rafRef.current) {
