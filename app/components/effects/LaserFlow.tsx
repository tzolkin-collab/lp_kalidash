...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    baseDprRef.current = Math.min(dpr ?? (window.devicePixelRatio || 1), isMobile ? 0.5 : 1);
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      if (avgFps < lowerThresh) next = clampN(currentDprRef.current * 0.85, dprFloor, base);
      else if (avgFps > upperThresh && currentDprRef.current < base)
        next = clampN(currentDprRef.current * 1.1, dprFloor, base);
    ...
    const animate = () => {
      ...
      if (!hasFadedRef.current) {
        ...
        (uniforms.uFade.value as number) = fade;
  ...
  useEffect(() => {
    ...
    if (!uniforms) return;
    (uniforms.uWispDensity.value as number) = wispDensity;
    (uniforms.uTiltScale.value as number) = mouseTiltStrength;
    (uniforms.uBeamXFrac.value as number) = horizontalBeamOffset;
    (uniforms.uBeamYFrac.value as number) = verticalBeamOffset;
    (uniforms.uFlowSpeed.value as number) = flowSpeed;
    (uniforms.uVLenFactor.value as number) = verticalSizing;
    (uniforms.uHLenFactor.value as number) = horizontalSizing;
    (uniforms.uFogIntensity.value as number) = fogIntensity;
    (uniforms.uFogScale.value as number) = fogScale;
    (uniforms.uWSpeed.value as number) = wispSpeed;
    (uniforms.uWIntensity.value as number) = wispIntensity;
    (uniforms.uFlowStrength.value as number) = flowStrength;
    (uniforms.uDecay.value as number) = decay;
    (uniforms.uFalloffStart.value as number) = falloffStart;
    (uniforms.uFogFallSpeed.value as number) = fogFallSpeed;
